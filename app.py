...
Terima gambar → kembalikan label (Anorganik / B3 / Organik) + confidence + Tips Daur Ulang.
"""
...
# Import inference engine (file inference.py harus berada di folder yang sama)
...
# Tim Backend wajib memasukkan GEMINI_API_KEY di environment server untuk mengaktifkan fitur tips daur ulang dari Gemini AI.
...
async def predict_single(file: UploadFile = File(...), token: int = Form(0)):
    ...
    try:
        ...
        if token != 0:
            ...
            tips = "Token Anda tidak valid. Tips daur ulang tidak tersedia."
    ...
    return PredictionResult(
        ...
        tips_daur_ulang=tips,  # Tambahan hasil Gemini
